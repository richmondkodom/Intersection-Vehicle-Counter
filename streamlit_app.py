...
# === Custom Toast Styling (bottom-right) ===
st.markdown("""
<style>
/* Move Streamlit toast container to bottom-right */
...
.stButton > button, .stDownloadButton > button {
    background-color: #2563eb; color: white !important; border-radius: 8px; border: none;
    padding: 0.6em 1.2em; font-weight: 600; cursor: pointer; transition: 0.3s;
}
.stButton > button:hover, .stDownloadButton > button:hover {
    background-color: #1e40af; color: #f1f5f9 !important;
}
</style>
"""
...
class CentroidTracker:
    ...
    def update(self, detections):
        ...
        assigned, out = set(), {}
        for dcx, dcy, w, h, cname, conf in detections:
            ...
            if best_id and best_dist<=self.max_distance:
                tr=self.tracks[best_id]; tr.trace.append((dcx,dcy)); tr.last_seen=now
                if tr.cls is None: tr.cls=cname
                assigned.add(best_id); out[best_id]=(dcx,dcy,w,h,tr.cls or cname,conf)
            else:
...
def detect_vehicles(frame, conf_thresh=0.2, nms_thresh=0.4, target_classes=None, input_size=416):
    ...
    for out in outs:
        for det in out:
            scores=det[5:]; class_id=int(np.argmax(scores)); confidence=float(scores[class_id])
    ...
    if len(idxs)>0:
        for i in idxs.flatten():
            x,y,bw,bh=boxes[i]; cx,cy=x+bw//2,y+bh//2
...
with st.sidebar:
    ...
    line_mode = st.selectbox("Which lines to use?", ["Horizontal & Vertical","Horizontal only","Vertical only"])
    ...
    st.markdown("**Classes**")
    selected_classes = st.multiselect("Vehicle classes", sorted(list(VEHICLE_CLASSES)), default=list(VEHICLE_CLASSES))

    draw_boxes = st.checkbox("Draw boxes", True)
    show_ids = st.checkbox("Show track IDs", True)
    show_trace = st.checkbox("Draw motion trails", True)
    fps_display = st.checkbox("Show FPS", True)

uploaded_video, cap = None, None
if source=="Upload Video":
    uploaded_video=st.file_uploader("Upload a video",type=["mp4","mov","avi","mkv"])
else:
    cam_index=st.number_input("Webcam index",0,step=1)
...
dashboard_placeholder=st.empty()  # main dashboard

if start_btn:
    if source=="Upload Video":
        if uploaded_video is None: st.warning("Please upload a video."); st.stop()
        tfile=tempfile.NamedTemporaryFile(delete=False); tfile.write(uploaded_video.read())
        cap=cv2.VideoCapture(tfile.name)
    else:
        cap=cv2.VideoCapture(int(cam_index))
    if not cap.isOpened(): st.error("Could not open source."); st.stop()

    tracker=CentroidTracker(max_distance=max_distance,max_age=max_age)
    frame_holder=st.empty(); fps_time=time.time(); frame_idx=0
    ...
    while cap.isOpened():
            ...
            if len(tr.trace)>=2:
                ...
                if use_h and not tr.counted_crossings["h"]:
                    if (py<h_line_y<=cy) or (py>h_line_y>=cy):
                        if dy>0: direction_counts["up_to_down"]+=1; events.append((tid,"up_to_down",tr.cls,frame_idx,event_time))
                        else: direction_counts["down_to_up"]+=1; events.append((tid,"down_to_up",tr.cls,frame_idx,event_time))
                        class_totals[tr.cls]+=1; tr.counted_crossings["h"]=True
                if use_v and not tr.counted_crossings["v"]:
                    if (px<v_line_x<=cx) or (px>v_line_x>=cx):
                        if dx>0: direction_counts["left_to_right"]+=1; events.append((tid,"left_to_right",tr.cls,frame_idx,event_time))
                        else: direction_counts["right_to_left"]+=1; events.append((tid,"right_to_left",tr.cls,frame_idx,event_time))
                        class_totals[tr.cls]+=1; tr.counted_crossings["v"]=True

        overlay_lines=[
            " | ".join([f"{cls.capitalize()}: {cnt}" for cls,cnt in class_totals.items()]),
            f"Total: {sum(class_totals.values())}",
        ...
        for i,line in enumerate(overlay_lines):
            cv2.putText(frame,line,(10,40+i*25),cv2.FONT_HERSHEY_SIMPLEX,0.8,(255,255,255),2,cv2.LINE_AA)
        ...
        with dashboard_placeholder.container():
            st.subheader("📊 Live Direction Dashboard")
            st.bar_chart(dash_df.set_index("Direction"))
            if dash_df["Count"].sum()>0:
                st.subheader("🍩 Distribution")
                st.dataframe(dash_df,use_container_width=True)
                st.pyplot(dash_df.set_index("Direction").plot.pie(y="Count",autopct="%1.0f%%").get_figure())
    ...
    cap.release(); frame_holder.empty()
    st.success("✅ Finished Processing")
    st.metric("Grand Total",sum(direction_counts.values()))
    if events:
        df=pd.DataFrame(events,columns=["track_id","direction","class","frame","timestamp"])
        st.dataframe(df,use_container_width=True)
        st.download_button("📥 Download CSV",df.to_csv(index=False).encode(),"counts.csv","text/csv")
